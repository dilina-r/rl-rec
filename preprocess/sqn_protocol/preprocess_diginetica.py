...
if __name__ == '__main__':
    ...
    events = pd.read_csv(os.path.join(PATH_TO_ORIGINAL, 'train-item-views.csv'), dtype={"sessionId": int, "userId": np.float64, "itemId": int}, sep=';')

    sessionid_eventdate_min = events.groupby(["sessionId"], as_index=False).agg(eventdatemin=("eventdate", "min"))
    events = pd.merge(events, sessionid_eventdate_min, on="sessionId", how="inner")
    events["time"] = (pd.to_datetime(events.eventdatemin).astype(int) / 10**9).astype(int) * 1000 + events.timeframe
    
    event_df = pd.DataFrame({
            "session_id": events.sessionId.values,
            "item_id": events.itemId.values,
            "timestamp": (events.time.values/1000).astype(int),
        })
    event_df['is_buy'] = 0
