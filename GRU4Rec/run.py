...
def load_data(fname, args):
    if fname.endswith('.pickle'):
        ...
        data = pd.read_csv(fname, sep='\t', dtype={args.session_key:'int32', args.item_key:'str'})
    return data
...
if args.test is not None:
    if args.primary_metric.lower() == 'recall':
        pm_index = f'HR@{args.measure[-1]}'
    elif args.primary_metric.lower() == 'mrr':
        pm_index = f'MRR@{args.measure[-1]}'
    else:
    ...
    for test_file in args.test:
        ...
        if args.log_primary_metric:
            print('PRIMARY METRIC: {}'.format(res.get_total_metric(pm_index)))
